import {
  ...
  directionSetter,
  getTranslate3d,
  firstVisibleItemWidth,
  lastVisibleVisibleWidth,
} from '../utils';
...
export const shiftSlideNonDirAutoWidth = params => {
  ...
  const padding = firstVisibleItemWidth(params);
  const distance = sliderMainWidth - padding;

  const result = directionSetter({
    rtl,
    input: Math.abs(getTranslate3d(sliderItems)) - distance,
  });

  const firstChildWidth = sliderItems.firstChild.clientWidth;

  if (
    !infinite &&
    // If more than half of firstChild is in viewPort
    (firstChildWidth / 2 > Math.abs(result) + Math.abs(firstItemPosition) ||
...
export const shiftSlideIsDirAutoWidth = params => {
  ...
  const padding = lastVisibleVisibleWidth(params);
  const distance = sliderMainWidth - padding;
