...
const lastVisibleItemPosition = params => {
  const { rtl, sliderItems, slider, sliderMainWidth } = params;
  const sliderRect = slider.getBoundingClientRect();
  const rects = vdomArrayConvertor(sliderItems.children).map(item => item.getBoundingClientRect());
  return rects.find(rect => {
    if (rtl) {
      return rect.x <= sliderRect.x;
    }
    return rect.x <= sliderRect.x + sliderMainWidth;
  });
};

export const firstVisibleItemWidth = params =>
  lastVisibleVisibleWidth({ ...params, rtl: !params.rtl });

export const lastVisibleVisibleWidth = params => {
  const { rtl, sliderMainWidth, slider } = params;
  const lastItemRect = lastVisibleItemPosition(params);
  if (!lastItemRect) {
    return 0;
  }
  const sliderRect = slider.getBoundingClientRect();
  const relativeX = lastItemRect.x - sliderRect.x;
  if (rtl) {
    // relativeX is negative here
    return lastItemRect.width + relativeX;
  }
  return sliderMainWidth - relativeX;
};
