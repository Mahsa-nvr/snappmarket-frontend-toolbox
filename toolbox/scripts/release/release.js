...
(async function run() {
  try {
    const { version } = await grabVersionAndGenerateNewOne(
      passedVersion,
    ...
    if (
      shell.exec(
        `lerna run --parallel change-version -- -- ${version} && lerna run --parallel build && lerna run --parallel publish-npm -- -- ${version}`,
      ).code !== 0
    ) {
